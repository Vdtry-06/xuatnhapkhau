...
export default function AdminAgentsPage() {
  ...
  const fetchAgents = async () => {
    ...
    try {
      ...
      flash('Lỗi tải danh sách đại lý', false);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleCreate = async (e) => {
    ...
    try {
      ...
      flash('Tạo đại lý và tài khoản đăng nhập thành công');
      setShowForm(false);
      setForm({ name: '', email: '', phone: '', address: '', taxCode: '', username: '', password: '' });
      fetchAgents();
    } catch (err) {
      flash(err.response?.data?.message || 'Lỗi tạo đại lý', false);
    }
  };
  ...
  return (
    <div style={s.page}>
      ...
      <div style={s.container}>
        <div style={s.header}>
          <h1 style={s.h1}>Quản lý đại lý con</h1>
          <button style={s.btnPrimary} onClick={() => setShowForm(!showForm)}>
            {showForm ? 'Đóng' : '+ Thêm đại lý'}
          </button>
        </div>
        ...
        {showForm && (
          <form style={s.form} onSubmit={handleCreate}>
            <h3 style={s.formTitle}>Tạo đại lý + tài khoản đăng nhập</h3>
            <div style={s.grid}>
              <input style={s.input} placeholder="Tên đại lý *" value={form.name}
            ...
            <button style={s.btnPrimary}>Lưu đại lý</button>
          </form>
        )}
        ...
        {loading ? <div style={s.center}>Đang tải...</div> : (
          <div style={s.tableWrap}>
            <table style={s.table}>
              <thead>
                <tr style={s.thead}>
                  {['ID', 'Tên đại lý', 'Email', 'Điện thoại', 'Mã số thuế', 'Trạng thái'].map(h =>
